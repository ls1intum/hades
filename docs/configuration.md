...
| `REQUEUE_DELAY` | `2s` | How often the operator re-reconciles a running `BuildJob` (Go duration). The operator does not watch Pods, so container transitions and job completion are only observed on these requeues; lower it for faster completion detection at the cost of more API traffic. 0 or invalid falls back to the default. | `HadesScheduler/HadesOperator/cmd/main.go` (`OperatorConfig`) |
| `LOG_DRAIN_TIMEOUT` | `45s` | How long a completed `BuildJob` is kept while its container logs drain, before it is deleted anyway (Go duration). Only relevant with `DELETE_ON_COMPLETE=true`. 0 or invalid falls back to the default. | `HadesScheduler/HadesOperator/cmd/main.go` (`OperatorConfig`) |
