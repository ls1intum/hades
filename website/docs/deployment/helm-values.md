...
| hadesLogManager.artemisAdapterUrl | string | `"http://hades-artemis-adapter.hades-artemis-connector.svc.cluster.local:8082/adapter/logs"` | External Artemis adapter endpoint the log manager forwards aggregated job logs to. |
...
| hadesOperator.DeleteOnComplete | bool | `true` | Delete BuildJob CRs (and their Jobs) once they finish. |
...
| hadesScheduler.configMode | string | `"operator"` | Kubernetes client mode: `operator` (default) or `serviceaccount` (legacy). |
