...
| tracing.jaeger.ui.ingress.className | string | `"nginx"` | Ingress class for the UI (basic auth is implemented with nginx ingress annotations). |
| tracing.jaeger.ui.ingress.enabled | bool | `false` | Expose the Jaeger UI via an Ingress (behind basic auth). |
