...
The HTTP APIs are documented with swaggo annotations; `HadesAPI/docs/` and `HadesLogManager/docs/` are generated (committed) and the Swagger UI is served at `/swagger/index.html` only when `DEBUG=true`.
